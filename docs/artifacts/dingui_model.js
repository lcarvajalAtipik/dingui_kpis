...
const P = {
  ...
  jun27: 50000,       // ventas de junio de 2027, € con IVA (usuario 17/09); costes variables en % de la temporada 2026, cobros con la estructura de julio
  cogs27: 21.75,      // % de ventas: compras 2026 netas de las devoluciones de fin de temporada (113.800 / 523.226)
  ...
  pepsi27: 0,         // € sin IVA, octubre: Pepsi no vuelve a pagar en 2027 (usuario 17/09)
...
const GRUPOS = [
  ["Temporada 2027", [
    ...
    ["jun27","Ventas de junio 2027","€ con IVA",5000,"Supuesto del usuario: 50.000 €. Costes variables en % de la temporada 2026 (compras, personal, DJs, RRPP 2,40, marketing 1,15, equipo 1,42, financiero 0,15); cobros con la estructura de julio"],
    ["cogs27","Compras de producto","% ventas",0.5,"2026: 24,2 bruto; 21,75 neto de devoluciones de fin de temporada (Melgarejo, Merino)"],
    ["personal27","Personal (con IRPF y SS)","% ventas",0.5,"2026: 18,56 de la temporada"],
  ...
  ["Acuerdos e impuestos", [
    ["pepsi27","Pepsi 2027","€ sin IVA",1000,"2026: 17.000. No se repite en 2027 (usuario 17/09)"],
...
const PATRON = {   // [mes, fila, label, importe 2026, escala con ventas?]
  ...
  sep: [["in_tpv","Cola del TPV de las últimas noches (2026: 21.075)", 21075, true],
        ["op_cogs","Bebida de agosto pagada en septiembre menos devoluciones cobradas (2026: 22.257 + 5.944 + 4.554 − 5.818 − 5.987)", -20950, true],["op_personal","Adelantos y resto de nómina de agosto (2026: 9.400 + 5.600)", -15000, true],
...
function modelo(p){
  ...
  for (let m = 0; m < 12; m++){
    ...
    if (V > 0){
      ...
      if (m === 5){
        add(pl, "marketing", i, "1,15 % de las ventas (marketing de la temporada 2026)", -V*0.0115);
        add(pl, "rrpp", i, "2,40 % de las ventas (RRPP de la temporada 2026)", -V*0.0240);
      } else add(pl, "marketing", i, "Barter (500 € base)", -500);
      if (m === 6 || m === 7) add(pl, "marketing", i, `Foto, vídeo y gestión comercial como ${m === 6 ? "julio" : "agosto"} de 2026`, (m === 6 ? -600 : -4400)*s);
      if (m === 5){
        add(pl, "extra", i, "1,42 % de las ventas (equipo y varios de la temporada 2026)", -V*0.0142);
        add(pl, "financiero", i, "0,15 % de las ventas (comisiones de la temporada 2026)", -V*0.0015);
      } else {
        add(pl, "extra", i, m === 7 ? "Como agosto de 2026 (8.700 € con IVA)" : "Como julio de 2026 (562 € con IVA)", (m === 7 ? -7902/1.21*s : -464*s));
  ...
  if (p.jun27 > 0){
    ...
    add(caja, "op_extra", 17, "Equipo y varios: 1,42 % de las ventas de junio", -Vj/1.1*0.0142*1.21);
    add(caja, "op_rrpp", 20, "RRPP de junio: 2,40 % de las ventas de junio + IVA", -Vj/1.1*0.0240*1.21);
    add(caja, "op_marketing", 20, "Marketing de junio: 1,15 % de las ventas de junio + IVA", -Vj/1.1*0.0115*1.21);
